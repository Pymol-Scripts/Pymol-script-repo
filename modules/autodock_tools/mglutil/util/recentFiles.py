...
# $Header: /opt/cvs/python/packages/share1.5/mglutil/util/recentFiles.py,v 1.13 2008/09/26 16:50:47 sargis Exp $
#
# $Id: recentFiles.py,v 1.13 2008/09/26 16:50:47 sargis Exp $
import os, pickle
...
class RecentFiles:
    ...
    def __init__(self, masterApp, masterMenu, filePath=None, 
        ...
        self.gui(masterMenu, menuLabel, underline=underline)
    ...
    def add(self, filePath, cmdStr, category="Documents"):
        ...
        if os.path.exists(filePath):
            ...
            if [filePath,cmdStr] in self.categories[category]:
                ...
                self.mainMenu.delete(index+1)
            if len(self.categories[category]) > 9:
            ...
            self.mainMenu.insert(0,'command', label=filePath, 
    ...
    def gui(self, masterMenu, menuLabel, underline=None):
        self.mainMenu = Tkinter.Menu(masterMenu)
        masterMenu.insert_cascade(3, label=menuLabel,
