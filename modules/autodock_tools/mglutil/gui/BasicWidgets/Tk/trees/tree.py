...
########################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/mglutil/gui/BasicWidgets/Tk/trees/tree.py,v 1.40.2.5 2011/06/15 20:55:23 sanner Exp $
#
# $Id: tree.py,v 1.40.2.5 2011/06/15 20:55:23 sanner Exp $
#
...
below self"""
    ...
    def drawNodeLabel(self, x, y):
        ...
        try:
            ...
            label = self.label = Label(canvas, text=text,font=self.tree().font,
            ...
            label.bind("<Double-1>", self.doubleLabel1)
            label.bind("<Button-3>", self.button3OnLabel)
            # MS 09/10 move_cb is in TreeWithButtons !
            label.bind("<Motion>", tree.move_cb)
            if os.name == 'nt': #sys.platform == 'win32':
                 label.bind("<MouseWheel>", tree.lineUpDown)
            else:
                label.bind("<Button-4>", tree.lineUp)
                label.bind("<Button-5>", tree.lineDown)
