...
#########################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/mglutil/gui/__init__.py,v 1.9 2008/10/08 22:54:57 vareille Exp $
#
# $Id: __init__.py,v 1.9 2008/10/08 22:54:57 vareille Exp $
#
...
widgetsOnBackWindowsCanGrabFocus = (sys.platform != 'win32') # True, False
...
if rcFile is None:
    ...
    if sys.platform != 'win32':
        ...
        # this is link is used by the self-running saved vision network to find mgltools
        # (we create this each time we run mglutils,
        # this way the last running vision will be used)
        mgltoolsDir = os.path.split(os.path.split(os.path.split(os.path.split(__file__)[0])[0])[0])[0]
        pythonshFile = mgltoolsDir + os.sep + 'share' + os.sep + 'bin' + os.sep + 'pythonsh'
        if os.path.isfile(pythonshFile) is False:
            pythonshFile = os.path.realpath(sys.executable)
