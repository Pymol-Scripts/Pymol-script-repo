...
##
##$Id: BugReport.py,v 1.13 2008/02/19 17:53:52 sargis Exp $
...
class BugReportCommand:
    ...
    def showuploadpage_cb(self,sumcont,desccont,atcont,email_ent,
        ...
        if len(atcont)>0:
            ...
            for f in filelist:
                if len(f)>=1:
                    params1 =  {"Bugzilla_login":"anonymous_bugzilla@yahoo.com","Bugzilla_password":"mgltools","bugid":"%i" %int(idnum),"action":"insert","description":"file  attached","ispatch":"0","contenttypemethod":"autodetect","comment":" ","obsolete":"",'contenttypeselection':"",'contenttypeentry':"",'data':open(f)}
                    ###################
                    #HTTP + MULTIPART 
                    ########################
                    import urllib2
