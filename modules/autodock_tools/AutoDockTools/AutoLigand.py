...
#########################################################################
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/AutoLigand.py,v 1.8.4.2 2009/08/21 19:38:15 sargis Exp $
#
# $Id: AutoLigand.py,v 1.8.4.2 2009/08/21 19:38:15 sargis Exp $
import sys, string
try:
...
def usage():
	...
	print "Usage:  python AutoLigand.py FileBaseName #_of_pts [x y z] [i j k]"
	print "    FileBaseName = just the name part from map files (FileBaseName.C.map)"
	print "    #_of_pts = number of fill points to use (int), (6 points per non-H atom)"
	...
	print "              when second point is input, the fill must contain both"
	print "    -out-progress (optional): output fill progress"
if len(sys.argv) < 3:
	usage()
	sys.exit(2)
...
if len(sys.argv)-offset == 6:
	start_point_flag = 1
	x_input = float(sys.argv[3])
	y_input = float(sys.argv[4])
	z_input = float(sys.argv[5])
elif len(sys.argv)-offset == 9:
	start_point_flag = 2
	x_input = float(sys.argv[3])
	y_input = float(sys.argv[4])
	z_input = float(sys.argv[5])
	x2_input = float(sys.argv[6])
	y2_input = float(sys.argv[7])
	z2_input = float(sys.argv[8])
	
every = 3 # note, if this spacing is changed, the StartPointModList must be changed below
# in this version of AutoLigand, only C, OA, and HD are used - the user can also add A, N, NA, SA
more_atom_types = 0
try:
	User_parameters = open('User_parameters.txt','r')
	UPflag = 1
	lineEvery = User_parameters.readline()
	wordEvery = string.split(lineEvery)
	every = string.atoi(wordEvery[0])
	lineNF = User_parameters.readline()
	wordNF = string.split(lineNF)
	Number_fills = string.atoi(wordNF[0])
	lineMAT = User_parameters.readline()
	wordMAT = string.split(lineMAT)
	more_atom_types = string.atoi(wordMAT[0])
...
if start_point_flag == 0:
	...
	if UPflag == 0:
		Number_fills = 10  # this is the defult value - can be changed by user in parameter file
