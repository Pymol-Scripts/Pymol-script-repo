...
# $Header: /opt/cvs/Support/update.py,v 1.29.2.4 2009/03/19 18:17:46 sargis Exp $
# $Id: update.py,v 1.29.2.4 2009/03/19 18:17:46 sargis Exp $
...
class Update:
    ...
    def __init__(self):
        if sys.platform == 'linux2':
            ...
            uname  = os.uname()
            if uname[-1] == "i386":
                self.sys_prefix = "i86Darwin"+uname[2][0]
            else:
                self.sys_prefix = "ppcDarwin"+uname[2][0]            
            self.update_file = self.sys_prefix+'.tar.gz'
        else:
    ...
    def getDate(self, nightly_url, update_url):
        "This funtion is used to extract date from remote directory listing"
        f = urllib.urlopen(nightly_url)
        html = f.read()
