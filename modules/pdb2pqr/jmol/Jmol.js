...
with (_jmol) {
 ...
 try {
  if (top.location.href.indexOf("JMOLJAR=")>=0)
    jmoljar = top.location.href.split("JMOLJAR=")[1].split("&")[0];
 } catch(e) {
  // can't access top.location
 }
 try {
  useNoApplet = (top.location.href.indexOf("NOAPPLET")>=0);
 } catch(e) {
...
function _jmolLoadModel(targetSuffix,remoteURL,array,isError,errorMessage){
 ...
 // Jmol 12.0 version:
 jmolScript("load data 'model'" + array.join("\n") + "\nend 'model' " + _jmol.optionalscript,targetSuffix)
}
...
function jmolLoadAjax_MSA(key,value,optionalscript,targetSuffix){
 ...
 targetSuffix || (targetSuffix="0")
 optionalscript || (optionalscript="")
 optionalscript == 1 && (optionalscript=' {1 1 1}')
